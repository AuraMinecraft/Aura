...
public class EventCommand extends BaseCommand {
    ...
    public void answer(CommandSender sender, String answer) {
        ...
        switch (answer) {
            case "accept" -> {
                ...
                if (user == null) {
                    sender.sendMessage(
                            AuraGameMaster.getMessage("database_error")
                    );
                    return;
                }
    ...
    public void suggest(CommandSender sender, OnlinePlayer target, double aura, String string) {
        Component message = AuraGameMaster.getMiniMessage().deserialize(string);
    ...
    public void message(CommandSender sender, OnlinePlayer target, String string) {
        Component component = AuraGameMaster.getMiniMessage().deserialize(string);
    ...
    public void list(CommandSender sender, @Optional Integer page, @Optional String targetName) {
        ...
        if (targetName != null) {
            if (!sender.hasPermission("aura.gamemaster.list.other")) {
                sender.sendMessage(AuraGameMaster.getMessage("no_permission"));
                return;
            }
        ...
        if (targetName == null) {
            sender.sendMessage(AuraGameMaster.getMessage("no_player"));
            return;
        }
